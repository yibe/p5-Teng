...
subtest 'search_with_pager simple' => sub {
...
subtest 'search_with_pager last' => sub {
...
subtest 'search_by_sql_with_pager simple' => sub {
    my ($rows, $pager) = $db->search_by_sql_with_pager(
        'SELECT id, name FROM mock_basic',
        [],
        'mock_basic',
        {rows => 3, page => 1},
    );
...
subtest 'search_by_sql_with_pager last' => sub {
    my ($rows, $pager) = $db->search_by_sql_with_pager(
        'SELECT id, name FROM mock_basic WHERE id > ?',
        [0],
        'mock_basic',
        {rows => 3, page => 11},
    );
